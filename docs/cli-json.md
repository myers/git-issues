...
| `cwd_error`                  | 2    | `Cwd`                         | —                        |
| `probe_error`                | 1    | `Probe`                       | —                        |
| `git_remote_error`           | 1    | `GitRemoteError`              | —                        |
...
```sh
$ iss init --json
{"ok":false,"error":{"kind":"not_a_git_repo","message":"not a git repo: /tmp/foo","details":{"path":"/tmp/foo"}}}
```
...
records it. URL validation is git's responsibility — whatever git
rejects, we surface as `git_remote_error` (exit 1).
