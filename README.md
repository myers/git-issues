...
An agent-first issue tracker. CLI: `iss`.

The Rust binary covers the full verb set with `--json` on every
command, push/pull over standard git transport, and a merge
driver so two clones can edit issues offline and converge
...
You'll need a recent Rust toolchain on your PATH.

```bash
cargo install --git https://github.com/myers/git-issues.git iss
cd /your/git/repo
...
integration tests that spawn real subprocesses (install:
