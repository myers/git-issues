...
fn init_outside_any_git_repo_returns_typed_error() {
    ...
    // `Error::Git` rather than the (deleted) jj-probe's `NotAGitRepo`.
    // The binary's `preflight::git_repo` is what now maps a non-repo cwd
    // to the operator-facing `NotAGitRepo` (exit 2) — see the `iss init`
