...
//!   "this isn't a git repo"). Surfaces with a clear stderr line so a
...
enum Commands {
    ...
    Ready {
        ...
        /// the same git-ref CAS rejection that blocks two parallel
        /// claims of the same id rolls this call back too. v2.3
        /// (`agent-claim-atomic`).
        #[arg(long = "claim")]
        claim: bool,
    ...
    Update {
        ...
        /// Atomically claim the issue: set assignee = current git
        /// `user.name` AND set status = `in-progress` in one
        /// multi-op commit. Two parallel `--claim` calls on the
        /// same id are race-free — a git-ref CAS decides the
        /// winner, the loser sees a CAS-loss retry and re-reads ready.
    ...
    /// Author defaults to the git user identity (`Name <email>` per
    /// `git config user.name`/`user.email`); `--author <NAME>`
    /// overrides. Empty bodies are rejected at the CLI layer (exit 2)
    /// because an empty comment is almost certainly a user mistake.
    Comment {
...
enum CliError {
    /// Bubbled up from `iss-storage`. The `NotAGitRepo` variant gets
    ...
    /// `iss comment` couldn't resolve a comment author. Either git's
    ...
    /// name is already taken. Plain `git remote add` surfaces this via
    /// stderr containing "already exists"; we translate that one
    /// phrase to a typed preflight error (exit 2) so callers get a
    /// stable `kind` to branch on rather than having to grep git's
    /// stderr themselves.
    #[error("git remote already exists: {0}")]
    RemoteAlreadyExists(String),

    /// `iss remote rm <name>` was asked to remove a remote that
    /// doesn't exist. Plain `git remote remove` surfaces this via
    /// stderr containing "No such remote"; we translate to a typed
    /// preflight error (exit 2) for the same reason as
    /// `RemoteAlreadyExists`.
    ...
    /// `iss push` shelled out to plain `git push` and got a non-zero
    /// exit that wasn't one of the typed cases above. Runtime (exit
    /// 1) — surfaces git's stderr verbatim so the operator can see
    /// what git said.
    ...
    /// (exit 1) — surfaces git's stderr verbatim so the operator can
    /// see what git said.
    ...
    /// `.comments.jsonl` (read via `git cat-file blob <oid>`), so
    /// file-level conflict markers never appear on the operator path.
    /// Same rationale as `Unmergeable` above for keeping the variant
    /// defined.
    #[allow(dead_code)]
    ...
    /// A concurrent git-issues writer landed first; the storage
    /// layer's CAS on the per-issue ref lost the race (v3) or the
    /// legacy v2 working-copy checkout conflicted (v2). Runtime
...
impl CliError {
    ...
    ///
    /// - `2` — preflight / argument failure (this includes "not a git
    ///   repo", since the verb can't proceed without one).
    /// - `1` — runtime failure.
    fn exit_code(&self) -> u8 {
        match self {
            CliError::Storage(StorageError::NotAGitRepo(_)) => 2,
    ...
    fn kind(&self) -> &'static str {
        match self {
            CliError::Storage(StorageError::NotAGitRepo(_)) => "not_a_git_repo",
            ...
            CliError::GitRemoteError(_) => "git_remote_error",
...
fn run_init(json: bool) -> Result<(), CliError> {
    ...
    // surfaces the typed `NotAGitRepo` error (exit 2, "run inside a
    ...
    // carry `refs/jjf/*`. Add the git-issues namespace so subsequent
...
/// BEFORE shelling out to git, so user-typo / stdin-empty failures don't
...
/// fail mid-write against a ref namespace that was never seeded.
fn run_new(
    json: bool,
    title: String,
    file: Option<PathBuf>,
    ...
    // 4. Preflight: we're inside a git repo AND the `issues` bookmark
...
/// cwd, probe for the git repo + `issues` bookmark, then hand off to
...
/// shape), resolve the cwd, probe for the git repo + `issues` bookmark
...
/// reject an empty label (exit 2), canonicalize cwd, probe for the git
...
fn run_remote_add(json: bool, name: String, url: String) -> Result<(), CliError> {
    ...
    if !out.status.success() {
        ...
        return Err(CliError::GitRemoteError(stderr.trim().to_owned()));
    }
...
/// `refs/heads/*`, so the git-issues `refs/jjf/*` namespace stays empty on
...
fn ensure_jjf_fetch_refspec(cwd: &Path, remote: &str) -> std::io::Result<()> {
    ...
    // `git clone` (or `jj git clone`) wrote at clone time stays in place.
...
/// canonicalize cwd, then probe for the git repo + `issues` bookmark.
...
/// body, canonicalize cwd, probe for the git repo + `issues` bookmark,
/// resolve the author, then hand off to storage. We deliberately do
/// the purely-local checks (id parse, body read) BEFORE shelling out
/// for the bookmark probe so a user typo doesn't kick off a `git`
/// subprocess that we'd just throw away; the bookmark probe itself
/// runs before author resolution for the same reason (see the preflight
...
fn run_comment(
    ...
    // 3. Preflight: git repo + `issues` bookmark present. We run this
    // BEFORE author resolution so a non-git cwd surfaces the typed
...
///
/// Format matches git's standard commit `author` field (`Name
/// <email>`) so a comment author and the surrounding commit's
/// `author` line stay canonically identical for history walks.
///
/// Edge cases:
...
///   `name`. This matches git's own permissive behavior (it'll let
...
fn run_ls(
    ...
    // Preflight: cwd is a git repo AND `issues` bookmark exists. Same
...
fn run_ready(
    ...
    if claim {
        ...
        // calls race on the per-issue ref's git CAS. The loser's
        // CAS attempt fails, `Storage::claim` retries internally
        // against the freshly-read (now-claimed) record, and the
        // idempotent-reclaim / `AlreadyClaimed` semantics documented
        // on `Storage::claim` take it from there — no separate
        // orchestrator re-run is needed for the race itself.
...
fn classify_storage_push_error(remote: &str, e: StorageError) -> CliError {
    ...
    if matches!(parsed, CliError::GitPushError(_)) {
        return CliError::Storage(e);
    }
    parsed
}

/// Map `git push` stderr to a typed `CliError`. Keeps the
...
fn classify_push_error(remote: &str, stderr: String) -> CliError {
    // Unknown remote — git's canonical phrases ("No git remote named
    // ...", "does not appear to be a git repository", "Could not read
    // from remote repository", "remote ... not found"), any of which
    // means the same thing operationally. The `remote rm` verb's
    // mapper matches the same phrase; we reuse the kind across the
    // push / fetch transport split.
    ...
    // Authentication. git surfaces libcurl/libgit2 errors here; we
    // pattern-match on the lowercase form of the key tokens so
    ...
    CliError::GitPushError(stderr.trim().to_owned())
}
...
fn run_pull(json: bool, remote: String) -> Result<(), CliError> {
    let cwd: PathBuf = std::env::current_dir().map_err(CliError::Cwd)?;
    let cwd = std::fs::canonicalize(&cwd).map_err(CliError::Cwd)?;
    // `pull` uses the git-repo-only preflight: a fresh clone has no
...
fn run_pull_v3(json: bool, remote: &str, cwd: &Path) -> Result<(), CliError> {
    // Lazy auto-config: a fresh clone may not have the git-issues fetch
...
fn classify_storage_pull_error(remote: &str, e: StorageError) -> CliError {
    ...
    if matches!(parsed, CliError::GitFetchError(_)) {
        return CliError::Storage(e);
    }
    parsed
}
...
/// Map `git fetch` stderr to a typed `CliError`. Mirrors
...
fn classify_fetch_error(remote: &str, stderr: String) -> CliError {
    // git's fetch surface uses a slightly different phrase than its
    ...
    CliError::GitFetchError(stderr.trim().to_owned())
}
...
error: failed to push some refs\n";
    ...
    // lives in `crates/iss/tests/actor.rs`, which scopes every
