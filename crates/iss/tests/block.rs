...
fn block_happy_path_show_reports_blocked_and_reason() {
    ...
    let out = run_iss(&repo, &["block", &id, "--reason", "waiting on PR-42"]);
    assert!(
        out.status.success(),
        "iss block failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
...
fn block_json_envelope_shape() {
    ...
    assert!(
        out.status.success(),
        "iss block --json failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn block_then_unblock_round_trip() {
    ...
    let out = run_iss(&repo, &["block", &id, "--reason", "wait"]);
    ...
    let out = run_iss(&repo, &["unblock", &id]);
    assert!(
        out.status.success(),
        "iss unblock failed: stderr={}",
        String::from_utf8_lossy(&out.stderr)
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn ready_excludes_blocked_by_default() {
    ...
    let out = run_iss(&repo, &["block", &id_a, "--reason", "park"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
...
fn ready_include_blocked_re_includes_blocked() {
    ...
    let out = run_iss(&repo, &["ready", "--include-blocked"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains(&id_a),
...
fn ls_status_blocked_filters_correctly() {
    ...
    let out = run_iss(&repo, &["ls", "--status", "blocked"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn block_multiline_reason_errors_invalid_input() {
    ...
    let out = run_iss(&repo, &["block", &id, "--reason", "line one\nline two"]);
    assert!(
        !out.status.success(),
...
fn ready_json_envelope_with_blocked_visible_carries_reason() {
    ...
    let id_a = create_issue(&repo, "A");
    let out = run_iss(&repo, &["block", &id_a, "--reason", "waiting on x"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));

    let out = run_iss(&repo, &["ready", "--json", "--include-blocked"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
