...
//! (issue `e4e483b`). Drives the compiled `iss` binary against per-test
...
fn new_rejects_embedded_newline_in_title() {
    ...
    let out = run_iss(&repo, &["--json", "new", "-t", "foo\nbar"]);
    assert_eq!(
        out.status.code(),
        Some(2),
        "stderr: {}",
...
// not because iss lost the bytes. The defense-in-depth for any
...
fn new_rejects_empty_title_with_typed_envelope() {
    ...
    let out = run_iss(&repo, &["--json", "new", "-t", "   "]);
    assert_eq!(
        out.status.code(),
        Some(2),
        "stderr: {}",
...
fn update_rejects_embedded_newline_in_title() {
    ...
    let create = run_iss(&repo, &["new", "-t", "baseline"]);
    assert!(create.status.success());
    let id = String::from_utf8_lossy(&create.stdout).trim().to_owned();

    let out = run_iss(&repo, &["--json", "update", &id, "--title", "foo\nbar"]);
    assert_eq!(
        out.status.code(),
        Some(2),
        "stderr: {}",
...
// `crates/iss-storage/tests/integration.rs`.)
