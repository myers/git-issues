...
//!
//! Mirrors the hermetic-scratch style of `iss-storage`'s
...
fn init_on_fresh_jj_repo_succeeds_and_plants_v3_sentinel() {
    let repo = make_git_repo("init_fresh");
    assert!(
    ...
    let out = run_iss(&repo, &["init"]);
    assert!(
        out.status.success(),
        "iss init failed: code={:?} stderr={}",
        out.status.code(),
...
fn init_is_idempotent_when_run_twice() {
    let repo = make_git_repo("init_idempotent");

    let first = run_iss(&repo, &["init"]);
    assert!(
...
fn init_json_emits_expected_object() {
    let repo = make_git_repo("init_json");

    let out = run_iss(&repo, &["init", "--json"]);
    assert!(
        out.status.success(),
        "init --json failed: {}",
        String::from_utf8_lossy(&out.stderr)
...
fn init_json_error_envelope_on_non_jj_directory() {
    ...
    // documented error envelope, not the plain `iss: <text>` line. Pins
    // the contract in `docs/cli-json.md` for the `not_a_git_repo` kind.
    let dir = scratch_non_git("init_json_err_non_jj");

    let out = run_iss(&dir, &["--json", "init"]);
    assert!(!out.status.success(), "init should fail outside a jj repo");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn global_json_flag_works_before_subcommand_too() {
    ...
    let repo = make_git_repo("init_json_before");
    let out = run_iss(&repo, &["--json", "init"]);
    assert!(
        out.status.success(),
        "--json init failed: {}",
        String::from_utf8_lossy(&out.stderr)
