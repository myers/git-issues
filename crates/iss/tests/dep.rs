...
fn dep_add_default_kind_blocks_show_reports_edge() {
    ...
    assert!(
        out.status.success(),
        "iss dep add failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    ...
    let out = run_iss(&repo, &["show", &child]);
...
fn dep_add_all_four_kinds_round_trip() {
    ...
    for kind in &["blocks", "parent-child", "related", "discovered-from"] {
        ...
        assert!(
            out.status.success(),
            "iss dep add --kind {kind} failed: stderr={}",
            String::from_utf8_lossy(&out.stderr),
        );
    }
...
fn dep_rm_removes_only_named_kind() {
    ...
    let out = run_iss(&repo, &["dep", "rm", &child, &parent, "--kind", "blocks"]);
    assert!(
        out.status.success(),
        "iss dep rm failed: stderr={}",
        String::from_utf8_lossy(&out.stderr)
    );
...
fn dep_tree_prints_parent_child_hierarchy() {
    ...
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let out = run_iss(&repo, &["dep", "add", &c, &b, "--kind", "parent-child"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));

    let out = run_iss(&repo, &["dep", "tree", &a]);
    assert!(
        out.status.success(),
        "iss dep tree failed: stderr={}",
        String::from_utf8_lossy(&out.stderr)
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn dep_tree_json_envelope_carries_nested_structure() {
    ...
    let out = run_iss(&repo, &["--json", "dep", "tree", &a]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn new_with_inline_dep_kind_syntax() {
    ...
    assert!(
        out.status.success(),
        "iss new with --dep parent-child:<id> failed: stderr={}",
        String::from_utf8_lossy(&out.stderr)
    );
...
fn dep_add_phantom_target_exits_with_issue_not_found() {
    ...
    let show = run_iss(&repo, &["show", &child]);
...
fn new_with_phantom_dep_target_exits_with_issue_not_found() {
    ...
    let ls = run_iss(&repo, &["--json", "ls", "--status", "all"]);
...
fn ready_excludes_child_of_blocked_parent_via_cascade() {
    ...
    let out = run_iss(&repo, &["--json", "ready"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn dep_add_blocks_cycle_exits_2_with_dependency_cycle() {
    ...
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let out = run_iss(&repo, &["dep", "add", &b, &c]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));

    let out = run_iss(&repo, &["--json", "dep", "add", &c, &a]);
    assert_eq!(
        out.status.code(),
        Some(2),
        "expected exit 2, got code={:?} stderr={}",
...
fn show_json_dependencies_kind_field_stays_wire_spelling() {
    ...
    let out = run_iss(&repo, &["--json", "show", &child]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
    let v: serde_json::Value = serde_json::from_str(stdout.trim()).expect("valid json");
