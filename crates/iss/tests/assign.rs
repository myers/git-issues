...
fn show_assignee(repo: &Path, id: &str) -> serde_json::Value {
    ...
    assert!(
        out.status.success(),
        "iss show --json failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr)
    );
...
fn assign_happy_path_sets_assignee() {
    ...
    let out = run_iss(&repo, &["assign", &id, "alice"]);
    assert!(
        out.status.success(),
        "iss assign failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
...
fn assign_empty_name_clears_assignee() {
    ...
    let out = run_iss(&repo, &["assign", &id, "bob"]);
    ...
    let out = run_iss(&repo, &["assign", &id, ""]);
    assert!(
        out.status.success(),
        "iss assign <id> \"\" failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn assign_json_envelope_shape_on_set() {
    ...
    let out = run_iss(&repo, &["assign", "--json", &id, "carol"]);
    assert!(
        out.status.success(),
        "iss assign --json failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn assign_json_envelope_shape_on_unset() {
    ...
    let out = run_iss(&repo, &["assign", &id, "dave"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    std::thread::sleep(std::time::Duration::from_millis(1100));

    // Clear with `--json` empty name — envelope must carry
    // explicit `null`, NOT omit the field.
    let out = run_iss(&repo, &["assign", "--json", &id, ""]);
    assert!(
        out.status.success(),
        "iss assign --json <id> \"\" failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn assign_nonexistent_id_surfaces_issue_not_found() {
    ...
    let out = run_iss(&repo, &["--json", "assign", nonexistent, "alice"]);
...
fn assign_name_with_newline_surfaces_invalid_input() {
    ...
    let out = run_iss(&repo, &["--json", "assign", &id, "alice\nevil"]);
