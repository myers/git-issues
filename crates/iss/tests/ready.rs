...
fn ready_dep_chain_returns_only_unblocked_issues() {
    ...
    let out = run_iss(&repo, &["ready"]);
    assert!(
        out.status.success(),
        "iss ready failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
...
fn ready_json_limit_one_returns_single_element_array() {
    ...
    let out = run_iss(&repo, &["ready", "--json", "--limit", "1"]);
    assert!(
        out.status.success(),
        "iss ready --json --limit 1 failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn ready_label_filter_intersects_with_unblocked_set() {
    ...
    let out = run_iss(&repo, &["ready", "--label", "backend"]);
    assert!(
        out.status.success(),
        "iss ready --label backend failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn ready_nothing_unblocked_exits_zero_with_empty_output() {
    ...
    let b_id = {
        ...
        let out = run_iss(&repo, &["ls", "--json", "--status", "open"]);
    ...
    let out = run_iss(&repo, &["ready", "--json"]);
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn ready_in_jj_repo_without_issues_bookmark_exits_two_with_init_hint() {
    let repo = make_git_repo("ready_no_bookmark");
    let out = run_iss(&repo, &["ready"]);
    assert!(!out.status.success());
    assert_eq!(
        out.status.code(),
...
fn ready_json_error_envelope_on_non_jj_directory() {
    ...
    let out = run_iss(&dir, &["--json", "ready"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(2));
    assert!(
        out.stdout.is_empty(),
...
/// `iss ready` exhibits the same silent-drop-fix behavior as `iss
/// ls`: a corrupt `refs/jjf/issues/<id>` ref drops out of the
/// candidate set but stderr carries a `iss: warning:` line naming
...
fn ready_warns_on_corrupt_issue_ref() {
    ...
    assert!(
        ...
        "ready stderr must carry the `iss: warning:` header, got: {stderr:?}"
    );
    assert!(
...
fn ready_parent_flag_filters_to_parent_child_children() {
    ...
    let epic_id = parse_id_from_stdout(
        &run_iss_with_stdin(&repo, &["new", "--json", "-t", "Epic", "--type", "epic", "--slug", "demo-epic", "-F", "-"], b"")
            .stdout,
    );
    let child_id = parse_id_from_stdout(
        &run_iss_with_stdin(&repo, &["new", "--json", "-t", "child", "--parent", epic_id.as_str(), "-F", "-"], b"").stdout,
    );
    let _sibling_id = parse_id_from_stdout(
        &run_iss_with_stdin(&repo, &["new", "--json", "-t", "sibling", "-F", "-"], b"").stdout,
    );

    // Bare `ready` returns all three.
    let bare = run_iss(&repo, &["ready", "--json"]);
    ...
    let filtered = run_iss(&repo, &["ready", "--json", "--parent", epic_id.as_str()]);
    ...
    let by_slug = run_iss(&repo, &["ready", "--json", "--parent", "demo-epic"]);
...
fn ready_parent_composes_with_type_and_limit() {
    ...
    let epic_id = parse_id_from_stdout(
        &run_iss_with_stdin(&repo, &["new", "--json", "-t", "E", "--type", "epic", "--slug", "epic", "-F", "-"], b"").stdout,
    );
    // Two children: one bug, one feature.
    let _bug = run_iss_with_stdin(
        &repo,
        &["new", "--json", "-t", "bug-child", "--type", "bug", "--parent", epic_id.as_str(), "-F", "-"],
        b""
    );
    let _feat = run_iss_with_stdin(
        &repo,
    ...
    let out = run_iss(&repo, &["ready", "--json", "--parent", "epic", "--type", "bug"]);
    ...
    let out = run_iss(&repo, &["ready", "--json", "--parent", "epic", "--limit", "1"]);
    let arr: Vec<serde_json::Value> = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["title"].as_str().unwrap(), "bug-child");
}
...
fn ready_parent_unknown_handle_exits_two() {
    ...
    let out = run_iss(&repo, &["ready", "--parent", "no-such-slug"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
fn ready_parent_bad_hex_exits_one_issue_not_found() {
    ...
    let out = run_iss(&repo, &["--json", "ready", "--parent", "deadbee"]);
    assert!(
        !out.status.success(),
