...
mod common;
use common::{run_iss, run_iss_with_stdin, scratch, ISS_BIN};

fn make_git_repo_with_user(name: &str, user: &str) -> PathBuf {
...
fn make_initialized_repo_with_user(name: &str, user: &str) -> PathBuf {
    let repo = make_git_repo_with_user(name, user);
...
fn update_claim_sets_assignee_and_status_in_progress() {
    ...
    let out = run_iss(&repo, &["update", &id, "--claim"]);
    assert!(
        out.status.success(),
        "iss update --claim failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
...
fn update_claim_lands_one_commit_with_two_trailers() {
    ...
    assert!(
        out.status.success(),
        "iss update --claim failed: stderr={}",
        String::from_utf8_lossy(&out.stderr),
    );
...
fn update_claim_different_user_errors_already_claimed() {
    // Set up: alice claims. Then run a SECOND iss instance with
    ...
    let out = run_iss(&repo, &["--json", "update", &id, "--claim"]);
...
fn update_unclaim_clears_assignee_and_resets_status() {
    ...
    let out = run_iss(&repo, &["update", &id, "--unclaim"]);
    assert!(
        out.status.success(),
    ...
    assert!(
        ...
        "assignee line should be omitted after clearing, got: {stdout}"
    );
}
...
fn update_claim_and_unclaim_are_mutually_exclusive() {
    ...
    let id = create_issue(&repo, "x", &[]);
    let out = run_iss(&repo, &["update", &id, "--claim", "--unclaim"]);
...
fn ready_excludes_in_progress_by_default() {
    ...
    let out = run_iss(&repo, &["update", &a, "--claim"]);
    assert!(out.status.success());
...
fn ready_include_claimed_shows_in_progress_too() {
    ...
    let out = run_iss(&repo, &["ready", "--include-claimed", "--json"]);
    assert!(out.status.success());
    let v: serde_json::Value =
        serde_json::from_str(&String::from_utf8_lossy(&out.stdout)).unwrap();
    let arr = v.as_array().unwrap();
...
fn ready_claim_limit_one_picks_and_claims_atomically() {
    ...
    let out = run_iss(&repo, &["--json", "ready", "--claim", "--limit", "1"]);
    assert!(
        out.status.success(),
...
fn ready_claim_requires_limit_one() {
    ...
    // No limit flag.
    let out = run_iss(&repo, &["ready", "--claim"]);
    ...
    let out = run_iss(&repo, &["ready", "--claim", "--limit", "2"]);
...
fn parallel_ready_claim_limit_one_assigns_two_different_ids() {
    ...
    let t1 = std::thread::spawn(move || run_iss(&repo1, &["--json", "ready", "--claim", "--limit", "1"]));
    let t2 = std::thread::spawn(move || run_iss(&repo2, &["--json", "ready", "--claim", "--limit", "1"]));
