...
//! issue `7100b51`. Drives the compiled `iss` binary against per-test
...
fn new_slug_collision_against_closed_holder_is_exit_two() {
    ...
    assert!(first_out.status.success());
    let first_id = String::from_utf8_lossy(&first_out.stdout).trim().to_owned();
    let close_out = run_iss(&repo, &["close", &first_id]);
    assert!(
...
fn update_type_and_slug_lands_one_commit() {
    ...
    let new = run_iss(&repo, &["new", "-t", "baseline"]);
...
fn update_unset_slug_clears_field() {
    ...
    let out = run_iss(&repo, &["update", &id, "--unset-slug"]);
...
fn slug_and_unset_slug_are_mutually_exclusive() {
    ...
    let new = run_iss(&repo, &["new", "-t", "anything"]);
    let id = String::from_utf8_lossy(&new.stdout).trim().to_owned();
...
fn id_taking_verbs_accept_slug_show() {
    ...
    let by_slug = run_iss(&repo, &["show", "handle-me-slug"]);
    assert!(
...
fn id_taking_verbs_accept_slug_close_and_update_and_comment() {
    ...
    // close <slug>
    let close = run_iss(&repo, &["close", "lifecycle-slug"]);
    assert!(close.status.success());

    // open <slug>
    let open = run_iss(&repo, &["open", "lifecycle-slug"]);
    ...
    let lbl = run_iss(&repo, &["label", "add", "lifecycle-slug", "needs-review"]);
...
fn unknown_handle_surfaces_slug_not_found() {
    ...
    let out = run_iss(&repo, &["--json", "show", "no-such-handle"]);
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    let v: serde_json::Value = serde_json::from_str(stderr.trim()).unwrap();
...
fn ls_type_filter_or_semantics_and_slug_substring() {
    ...
    run_iss(&repo, &["new", "-t", "alpha", "--type", "bug", "--slug", "alpha-slug"]);
    run_iss(&repo, &["new", "-t", "beta", "--type", "feature", "--slug", "beta-slug"]);
    run_iss(&repo, &["new", "-t", "gamma"]);

    // --type bug returns alpha only.
    let bugs = run_iss(&repo, &["--json", "ls", "--type", "bug"]);
    ...
    let slug_filt = run_iss(&repo, &["--json", "ls", "--slug", "alpha"]);
...
fn show_json_includes_type_and_slug_fields() {
    ...
    let show = run_iss(&repo, &["--json", "show", &id]);
...
fn comment_stdin_via_slug_for_completeness() {
    ...
    run_iss(&repo, &["new", "-t", "slot", "--slug", "comment-via-slug"]);
...
fn show_full_id_resolves() {
    ...
    let out = run_iss(&repo, &["new", "-t", "full-id"]);
    let id = String::from_utf8_lossy(&out.stdout).trim().to_owned();
    let show = run_iss(&repo, &["--json", "show", &id]);
    assert!(
        show.status.success(),
