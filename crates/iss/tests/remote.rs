...
//! - non-jj cwd → exit 2 + `not_a_git_repo` (covers all three arms),
...
fn remote_add_happy_path_then_ls_lists_it() {
    let repo = make_git_repo("remote_add_happy");

    let out = run_iss(&repo, &["remote", "add", "origin", "https://example.com/x.git"]);
    assert!(
        out.status.success(),
    ...
    let out = run_iss(&repo, &["remote", "ls"]);
...
fn remote_add_and_ls_json_envelopes() {
    let repo = make_git_repo("remote_add_ls_json");
    ...
    let out = run_iss(&repo, &["remote", "--json", "ls"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
    let v: serde_json::Value =
...
fn remote_ls_empty_repo_silent_plain_and_empty_array_json() {
    let repo = make_git_repo("remote_ls_empty");
...
fn remote_add_then_rm_round_trip() {
    let repo = make_git_repo("remote_add_rm_round_trip");

    let out = run_iss(&repo, &["remote", "add", "origin", "https://example.com/x.git"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));

    let out = run_iss(&repo, &["remote", "rm", "origin"]);
    assert!(
        out.status.success(),
...
fn remote_rm_json_envelope_shape() {
    let repo = make_git_repo("remote_rm_json");

    let out = run_iss(&repo, &["remote", "add", "origin", "https://example.com/x.git"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));

    let out = run_iss(&repo, &["remote", "--json", "rm", "origin"]);
    assert!(
        out.status.success(),
...
fn remote_add_duplicate_exits_two_with_typed_kind() {
    let repo = make_git_repo("remote_add_duplicate");

    let out = run_iss(&repo, &["remote", "add", "origin", "https://example.com/x.git"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));

    // Second add with the same name — exit 2, plain stderr mentions the name.
    let out = run_iss(&repo, &["remote", "add", "origin", "https://example.com/y.git"]);
...
fn remote_rm_nonexistent_exits_two_with_typed_kind() {
    let repo = make_git_repo("remote_rm_nonexistent");

    // Plain stderr — exit 2, mentions the missing name.
    let out = run_iss(&repo, &["remote", "rm", "nope"]);
    ...
    let out = run_iss(&repo, &["--json", "remote", "rm", "nope"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    let v: serde_json::Value =
...
fn remote_add_in_non_jj_directory_exits_two() {
    // Must be outside the git-issues source tree — scratch() dirs live
    // inside the project's git repo and would pass the git preflight.
    let dir = scratch_non_git("remote_add_non_jj");
    let out = run_iss(&dir, &["remote", "add", "origin", "https://example.com/x.git"]);
...
fn remote_rm_in_non_jj_directory_exits_two() {
    ...
    let out = run_iss(&dir, &["remote", "rm", "origin"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(2));
}
...
fn remote_verbs_do_not_require_bugs_bookmark() {
    // The whole point of using the `git_repo`-only preflight (instead
    ...
    let repo = make_git_repo("remote_no_bugs_bookmark");
...
fn remote_add_help_documents_positionals() {
    ...
    let out = Command::new(ISS_BIN)
        ...
        .current_dir(cwd)
        .output()
        .expect("spawn iss remote add --help");
    assert!(out.status.success());
    let help = String::from_utf8_lossy(&out.stdout);
...
fn remote_add_ls_rm_in_plain_git_repo() {
    ...
    let add = run_iss(&tmp, &["remote", "add", "origin", "https://example.com/x.git"]);
    assert!(
        add.status.success(),
        "remote add failed: {}",
    ...
    let ls = run_iss(&tmp, &["remote", "ls", "--json"]);
    assert!(
        ls.status.success(),
        "remote ls failed: {}",
    ...
    let rm = run_iss(&tmp, &["remote", "rm", "origin"]);
    assert!(
        rm.status.success(),
        "remote rm failed: {}",
