...
fn label_add_happy_path_show_reports_label() {
    ...
    let out = run_iss(&repo, &["label", "add", &id, "backend"]);
    assert!(
        out.status.success(),
        "iss label add failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
...
fn label_add_then_rm_round_trip() {
    ...
    // rm → show omits the labels line entirely — the empty-set
    // sentinel `print_issue_plain` renders (v2.11: omit, not `(none)`).
    let out = run_iss(&repo, &["label", "rm", &id, "backend"]);
    assert!(
        out.status.success(),
        "iss label rm failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
    ...
    assert!(
        ...
        "after rm, show should omit the labels line, got: {stdout}"
    );
}
...
fn label_add_json_envelope_shape() {
    ...
    let out = run_iss(&repo, &["label", "--json", "add", &id, "frontend"]);
    assert!(
        out.status.success(),
        "iss label --json add failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
    ...
    let out = run_iss(&repo, &["show", "--json", &id]);
...
fn label_rm_json_envelope_shape() {
    ...
    let out = run_iss(&repo, &["label", "add", &id, "tooling"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));

    let out = run_iss(&repo, &["label", "--json", "rm", &id, "tooling"]);
    assert!(
        out.status.success(),
        "iss label --json rm failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn label_rm_absent_label_lands_trailer() {
    ...
    let out = run_iss(&repo, &["label", "rm", &id, "neverwas"]);
    assert!(
        out.status.success(),
...
fn label_json_error_envelope_on_empty_label() {
    ...
    let out = run_iss(&repo, &["--json", "label", "add", &id, ""]);
    assert!(!out.status.success());
...
fn label_add_empty_label_exits_two() {
    ...
    let out = run_iss(&repo, &["label", "add", &id, ""]);
...
fn label_rm_empty_label_exits_two() {
    ...
    let out = run_iss(&repo, &["label", "rm", &id, ""]);
    assert!(!out.status.success(), "empty label should fail");
    assert_eq!(out.status.code(), Some(2));
}
...
fn label_add_nonexistent_id_exits_one() {
    ...
    let out = run_iss(&repo, &["label", "add", nonexistent, "backend"]);
...
fn label_add_bad_id_exits_two() {
    ...
    for bad in ["short", "GGGGGGG", "12345", "not-a-real-bug-id"] {
        let out = run_iss(&repo, &["label", "add", bad, "backend"]);
...
fn label_rm_bad_id_exits_two() {
    ...
    let out = run_iss(&repo, &["label", "rm", "not-hex", "backend"]);
...
fn label_in_non_jj_directory_exits_two() {
    ...
    let out = run_iss(&dir, &["label", "add", "abcdef0", "backend"]);
...
fn label_add_in_jj_repo_without_bugs_bookmark_exits_two_with_init_hint() {
    let repo = make_git_repo("label_add_no_bookmark");
    let out = run_iss(&repo, &["label", "add", "abcdef0", "backend"]);
    assert!(!out.status.success());
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn label_rm_in_jj_repo_without_bugs_bookmark_exits_two_with_init_hint() {
    ...
    let repo = make_git_repo("label_rm_no_bookmark");
    let out = run_iss(&repo, &["label", "rm", "abcdef0", "backend"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
fn label_add_help_documents_positionals_and_json() {
    ...
    let out = Command::new(ISS_BIN)
        ...
        .current_dir(cwd)
        .output()
        .expect("spawn iss label add --help");
    assert!(out.status.success());
    let help = String::from_utf8_lossy(&out.stdout);
...
fn label_rm_help_documents_positionals_and_json() {
    ...
    let out = Command::new(ISS_BIN)
        ...
        .current_dir(cwd)
        .output()
        .expect("spawn iss label rm --help");
    assert!(out.status.success());
    let help = String::from_utf8_lossy(&out.stdout);
