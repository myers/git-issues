...
fn close_happy_path_show_reports_closed() {
    ...
    assert!(
        ...
        "show should report [closed] after iss close: {stdout}"
    );
}
...
fn close_json_envelope_shape() {
    ...
    let out = run_iss(&repo, &["close", "--json", &id]);
    assert!(
        out.status.success(),
        "iss close --json failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn close_then_open_round_trip() {
    ...
    let out = run_iss(&repo, &["open", &id]);
    assert!(
        out.status.success(),
        "iss open failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
    ...
    assert!(
        ...
        "show should report [open] after iss open: {stdout}"
    );
}
...
fn open_json_envelope_shape() {
    ...
    let out = run_iss(&repo, &["open", "--json", &id]);
    assert!(
        out.status.success(),
        "iss open --json failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn close_json_error_envelope_on_nonexistent_id() {
    ...
    let out = run_iss(&repo, &["--json", "close", nonexistent]);
    assert!(!out.status.success());
...
fn open_json_error_envelope_on_nonexistent_id() {
    ...
    let out = run_iss(&repo, &["--json", "open", nonexistent]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&out.stderr);
    let v: serde_json::Value =
...
fn close_nonexistent_id_exits_one() {
    ...
    let out = run_iss(&repo, &["close", nonexistent]);
...
fn close_bad_id_exits_two() {
    ...
    for bad in ["short", "GGGGGGG", "12345", "not-a-real-bug-id"] {
        let out = run_iss(&repo, &["close", bad]);
...
fn close_in_non_jj_directory_exits_two() {
    ...
    let out = run_iss(&dir, &["close", "abcdef0"]);
...
fn close_in_jj_repo_without_bugs_bookmark_exits_two_with_init_hint() {
    let repo = make_git_repo("close_no_bookmark");
    let out = run_iss(&repo, &["close", "abcdef0"]);
    assert!(!out.status.success());
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn open_in_jj_repo_without_bugs_bookmark_exits_two_with_init_hint() {
    ...
    // from the other's.
    let repo = make_git_repo("open_no_bookmark");
    let out = run_iss(&repo, &["open", "abcdef0"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
