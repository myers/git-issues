//! Integration tests for the four `iss` memory verbs (`remember`,
...
mod common;
use common::{scratch, run_iss, ISS_BIN};

/// Make a directory that's an initialized git+iss repo (i.e. the
...
fn make_initialized(name: &str) -> PathBuf {
    ...
    let out = Command::new("git")
        .args(["config", "user.email", "test@iss-tests.invalid"])
        .current_dir(&dir)
        .output()
...
fn run_iss_stdin(cwd: &Path, args: &[&str], stdin: &str) -> Output {
    use std::io::Write;
    ...
    child
        ...
        .unwrap();
    child.wait_with_output().expect("wait iss")
}

#[test]
fn remember_then_recall_round_trips() {
    let repo = make_initialized("memory_round_trip");
    let out = run_iss(&repo, &["remember", "always run tests with -race flag"]);
    assert!(
        out.status.success(),
        "remember failed: {}",
        String::from_utf8_lossy(&out.stderr)
...
fn remember_with_explicit_key_uses_it() {
    ...
    let recall = run_iss(&repo, &["recall", "dolt-phantoms"]);
...
fn remember_upserts_existing_key() {
    ...
    run_iss(&repo, &["remember", "first value", "--key", "kkk"]);
    let out = run_iss(&repo, &["remember", "second value", "--key", "kkk"]);
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
    ...
    let recall = run_iss(&repo, &["recall", "kkk"]);
    assert_eq!(
        String::from_utf8_lossy(&recall.stdout).trim(),
        "second value"
    );
...
fn forget_removes_memory() {
    ...
    run_iss(&repo, &["remember", "value", "--key", "doomed"]);
    let out = run_iss(&repo, &["forget", "doomed"]);
    assert!(out.status.success(), "forget failed: {}", String::from_utf8_lossy(&out.stderr));

    let recall = run_iss(&repo, &["recall", "doomed"]);
    assert!(
        !recall.status.success(),
...
fn forget_on_unknown_key_exits_1_with_memory_not_found() {
    ...
    let out = run_iss(&repo, &["forget", "no-such-key"]);
...
fn memories_lists_all_keys_alphabetically() {
    ...
    run_iss(&repo, &["remember", "zz", "--key", "zebra"]);
    run_iss(&repo, &["remember", "aa", "--key", "alpha"]);
    run_iss(&repo, &["remember", "mm", "--key", "middle"]);

    let out = run_iss(&repo, &["memories"]);
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn memories_filters_by_substring_case_insensitive() {
    ...
    let out = run_iss(&repo, &["memories", "AUTH"]);
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
    ...
    let out2 = run_iss(&repo, &["memories", "RACE"]);
...
fn memories_plain_text_is_one_line_per_entry_no_header() {
    ...
    run_iss(&repo, &["remember", "first fact", "--key", "alpha"]);
    run_iss(&repo, &["remember", "second fact", "--key", "beta"]);
...
fn memories_json_returns_bare_array() {
    ...
    run_iss(&repo, &["remember", "v1", "--key", "key-1"]);
    run_iss(&repo, &["remember", "v2", "--key", "key-2"]);

    let out = run_iss(&repo, &["memories", "--json"]);
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn recall_json_emits_envelope_with_found_field() {
    ...
    run_iss(&repo, &["remember", "v", "--key", "kkk"]);

    let out = run_iss(&repo, &["recall", "--json", "kkk"]);
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
    let v: serde_json::Value = serde_json::from_str(stdout.trim()).expect("valid json");
...
fn recall_unknown_key_exits_1() {
    ...
    let out = run_iss(&repo, &["recall", "no-such-thing"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(1));
}
...
fn remember_from_stdin_with_explicit_key() {
    ...
    let recall = run_iss(&repo, &["recall", "from-stdin"]);
    assert!(recall.status.success());
    assert_eq!(
        String::from_utf8_lossy(&recall.stdout).trim(),
        "value from stdin"
...
fn show_include_memories_appends_memory_block() {
    ...
    let new_out = run_iss(&repo, &["--json", "new", "-t", "the issue"]);
    assert!(
    ...
    run_iss(&repo, &["remember", "value 1", "--key", "alpha"]);
    run_iss(&repo, &["remember", "value 2", "--key", "beta"]);

    let out = run_iss(&repo, &["show", &id, "--include-memories"]);
    assert!(
        out.status.success(),
...
fn show_without_include_memories_does_not_append_block() {
    ...
    let new_v: serde_json::Value =
        serde_json::from_str(String::from_utf8_lossy(&new_out.stdout).trim())
            .expect("valid json");
    let id = new_v["id"].as_str().unwrap().to_owned();
    run_iss(&repo, &["remember", "value", "--key", "kkk"]);

    let out = run_iss(&repo, &["show", &id]);
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
...
fn remember_json_envelope_shape() {
    ...
    let out = run_iss(&repo, &["--json", "remember", "v", "--key", "kkk"]);
    assert!(out.status.success());
    ...
    let upsert = run_iss(&repo, &["--json", "remember", "v2", "--key", "kkk"]);
    let v: serde_json::Value =
...
fn forget_json_envelope_shape() {
    ...
    run_iss(&repo, &["remember", "v", "--key", "kkk"]);
    let out = run_iss(&repo, &["--json", "forget", "kkk"]);
    assert!(out.status.success());
    let v: serde_json::Value =
        serde_json::from_str(String::from_utf8_lossy(&out.stdout).trim())
            .expect("valid json");
...
fn forget_missing_key_json_error_envelope() {
    ...
    let out = run_iss(&repo, &["--json", "forget", "no-key"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&out.stderr);
