//! Acceptance test: full `iss` lifecycle with `jj` absent from PATH.
//!
...
/// Ephemeral scratch directory outside the git-issues source tree.
/// Cleaned up at the end of the test.
fn make_scratch(name: &str) -> PathBuf {
    let dir = std::env::temp_dir()
        .join("iss-no-jj-tests")
        .join(name);
    if dir.exists() {
...
fn full_lifecycle_without_jj_on_path() {
    ...
    assert!(
        ...
        "iss init failed with jj absent: code={:?} stderr={}",
    ...
    assert!(
        ...
        "iss new failed with jj absent: code={:?} stderr={}",
        new_out.status.code(),
    ...
    assert!(
        ...
        "iss ls failed with jj absent: code={:?} stderr={}",
        ls_out.status.code(),
    ...
    assert!(
        ...
        "iss show failed with jj absent: code={:?} stderr={}",
