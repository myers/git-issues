...
//! ticket `326bbf7`. Drives the compiled `iss` binary against
...
fn update_priority_then_unset_round_trips_through_both_states() {
    ...
    let out = run_iss(&repo, &["update", &id, "--priority", "0"]);
    assert!(
        out.status.success(),
    ...
    let out = run_iss(&repo, &["update", &id, "--unset-priority"]);
    assert!(
        out.status.success(),
...
fn out_of_range_priority_rejected_with_invalid_priority_envelope() {
    ...
    let out = run_iss(&repo, &["new", "-t", "oor", "-p", "5"]);
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn ls_priority_filter_returns_or_of_listed_values() {
    ...
    let out = run_iss(&repo, &["ls", "--json", "-p", "0", "-p", "2"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let v: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
