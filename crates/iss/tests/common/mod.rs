...
/// git-issues source tree (which is itself a git repo).
pub(crate) fn scratch_non_git(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join("iss-tests").join(name);
    if dir.exists() {
        fs::remove_dir_all(&dir).unwrap();
    }
    fs::create_dir_all(&dir).unwrap();
...
pub(crate) fn make_git_repo(name: &str) -> PathBuf {
...
pub(crate) fn run_iss(cwd: &Path, args: &[&str]) -> Output {
    Command::new(ISS_BIN)
        .args(args)
        .current_dir(cwd)
        .output()
...
pub(crate) fn run_iss_with_stdin(cwd: &Path, args: &[&str], stdin_bytes: &[u8]) -> Output {
