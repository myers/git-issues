...
fn create_issue(
    ...
    let out = run_iss_with_stdin(repo, &args, body);
    assert!(
        out.status.success(),
...
fn close_bug(repo: &Path, id: &str) {
    let out = run_iss(repo, &["close", id]);
    assert!(
        out.status.success(),
        "iss close failed during setup: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr)
    );
}
...
fn ls_default_status_open_returns_every_bug_when_all_open() {
    ...
    let out = run_iss(&repo, &["ls"]);
    assert!(
        out.status.success(),
        "iss ls failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
...
fn ls_status_filter_open_closed_all() {
    ...
    // --status closed → 1 row.
    let out = run_iss(&repo, &["ls", "--status", "closed"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let rows = parse_ls_rows(&String::from_utf8_lossy(&out.stdout));
    ...
    // --status all → 3 rows.
    let out = run_iss(&repo, &["ls", "--status", "all"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let rows = parse_ls_rows(&String::from_utf8_lossy(&out.stdout));
...
fn ls_label_filter_single_and_intersection() {
    ...
    let out = run_iss(&repo, &["ls", "--label", "x"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let rows = parse_ls_rows(&String::from_utf8_lossy(&out.stdout));
    let ids: Vec<&str> = rows.iter().map(|r| r.0.as_str()).collect();
    ...
    let out = run_iss(&repo, &["ls", "--label", "nope"]);
...
fn ls_json_emits_array_of_bug_records() {
    ...
    let out = run_iss(&repo, &["ls", "--json"]);
    assert!(
        out.status.success(),
        "iss ls --json failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn ls_in_jj_repo_without_bugs_bookmark_exits_two_with_init_hint() {
    ...
    // fire with the typed `run iss init first` message.
    let repo = make_git_repo("ls_no_bookmark");
    let out = run_iss(&repo, &["ls"]);
    assert!(!out.status.success());
    assert_eq!(
        out.status.code(),
...
/// `crates/iss-storage/tests/v3_read_path.rs` and `integration.rs`.
...
fn ls_warns_on_corrupt_issue_ref_plain_text() {
    ...
    // stderr names the casualty via a `iss: warning:` line.
...
fn ls_parent_flag_filters_to_parent_child_children() {
    ...
    let epic_id = parse_id_from_stdout(
        &run_iss(&repo, &["new", "--json", "-t", "Epic", "--type", "epic", "--slug", "demo"]).stdout,
    );
    let child_id = parse_id_from_stdout(
        &run_iss(&repo, &["new", "--json", "-t", "child", "--parent", epic_id.as_str()]).stdout,
    );
    let _sibling = run_iss(&repo, &["new", "--json", "-t", "sibling"]);

    let out = run_iss(&repo, &["ls", "--json", "--parent", "demo"]);
...
fn ls_parent_unknown_handle_exits_two() {
    ...
    let out = run_iss(&repo, &["ls", "--parent", "no-such-slug"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(2));
}
...
fn ls_parent_bad_hex_exits_one_issue_not_found() {
    ...
    let out = run_iss(&repo, &["--json", "ls", "--parent", "deadbee"]);
    assert!(
        !out.status.success(),
...
fn ls_parent_composes_with_type_and_status() {
    ...
    let epic_out = run_iss(&repo, &["new", "--json", "-t", "E", "--type", "epic", "--slug", "epic-parent"]);
    assert!(
    ...
    let bug_out = run_iss(&repo, &["new", "--json", "-t", "bug", "--type", "bug", "--parent", epic_id.as_str()]);
    assert!(
    ...
    let feat_out = run_iss(&repo, &["new", "--json", "-t", "feat", "--type", "feature", "--parent", epic_id.as_str()]);
    assert!(
    ...
    run_iss(&repo, &["close", bug_id.as_str()]);

    let out = run_iss(&repo, &["ls", "--json", "--parent", "epic-parent", "--type", "bug"]);
    assert!(
        out.status.success(),
        "ls failed: stderr={}",
        String::from_utf8_lossy(&out.stderr)
    ...
    let out = run_iss(&repo, &["ls", "--json", "--parent", "epic-parent", "--type", "bug", "--status", "all"]);
    assert!(
        out.status.success(),
...
fn ls_meta_bare_key_rejected_at_clap_parse_time() {
    ...
    let out = run_iss(&repo, &["ls", "--meta", "foo"]);
    assert!(
        !out.status.success(),
