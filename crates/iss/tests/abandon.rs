...
fn abandon_happy_path_show_reports_abandoned() {
    ...
    let out = run_iss(&repo, &["abandon", &id]);
    assert!(
        out.status.success(),
        "iss abandon failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    ...
    assert!(
        ...
        "show should report [abandoned] after iss abandon: {stdout}"
    );
}
...
fn abandon_json_envelope_shape() {
    ...
    let out = run_iss(&repo, &["abandon", "--json", &id]);
    assert!(
        out.status.success(),
        "iss abandon --json failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn ls_default_hides_abandoned_issues() {
    ...
    let out = run_iss(&repo, &["abandon", &nuke]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
...
fn ls_status_abandoned_lists_only_abandoned() {
    ...
    let out = run_iss(&repo, &["ls", "--status", "abandoned"]);
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains(&nuke),
...
fn abandon_json_error_envelope_on_nonexistent_id() {
    ...
    let out = run_iss(&repo, &["--json", "abandon", nonexistent]);
    assert!(!out.status.success());
...
fn abandon_bad_id_exits_two() {
    ...
    for bad in ["short", "GGGGGGG", "12345", "not-a-real-bug-id"] {
        let out = run_iss(&repo, &["abandon", bad]);
...
fn abandon_in_non_jj_directory_exits_two() {
    ...
    let out = run_iss(&dir, &["abandon", "abcdef0"]);
...
fn abandon_in_jj_repo_without_bugs_bookmark_exits_two_with_init_hint() {
    let repo = make_git_repo("abandon_no_bookmark");
    let out = run_iss(&repo, &["abandon", "abcdef0"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
fn update_status_open_revives_an_abandoned_issue() {
    ...
    let out = run_iss(&repo, &["update", &id, "--status", "open"]);
    assert!(
        out.status.success(),
