...
/// Build a git repo with the given user.name + user.email pinned in
/// repo-local config. Pass `user = None` to leave user.name unset
/// (for the "no current user" / chain-runs-dry tests).
fn make_git_repo_with_user(name: &str, user: Option<&str>) -> PathBuf {
...
fn run_iss_with_env(cwd: &Path, args: &[&str], env: &[(&str, Option<&str>)]) -> Output {
...
fn run_iss_with_stdin_env(
    ...
    let mut child = cmd.spawn().expect("spawn iss");
    child
        .stdin
        .as_mut()
        .expect("stdin handle")
...
fn create_issue(repo: &Path, title: &str, extra_args: &[&str]) -> String {
    ...
    let out = run_iss_with_stdin_env(repo, &args, b"", &[("ISS_ACTOR", None)]);
    assert!(
        out.status.success(),
...
fn show_assignee(repo: &Path, id: &str) -> Option<String> {
    let out = run_iss_with_env(repo, &["show", "--json", id], &[("ISS_ACTOR", None)]);
    assert!(
        out.status.success(),
        "iss show failed: stderr={}",
        String::from_utf8_lossy(&out.stderr)
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn claim_all_slots_empty_errors_no_current_user() {
    ...
    cmd.args(["new", "--json", "-t", "no-user", "-F", "-"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    let mut child = cmd.spawn().expect("spawn iss new");
    child
        .stdin
        .as_mut()
        .expect("stdin")
...
fn comment_env_drives_author_when_flag_absent() {
    ...
    let out = run_iss_with_stdin_env(
        &repo,
        &["comment", &id, "-F", "-"],
...
fn claim_uses_git_config_user_name() {
    // git init (required for iss init), but do NOT set jj user config.
    let root = make_git_repo_with_user("claim_uses_git_config", None);
    ...
    assert!(out.status.success(), "iss new failed: {}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8_lossy(&out.stdout);
    let v: serde_json::Value = serde_json::from_str(stdout.trim()).expect("parse new --json");
