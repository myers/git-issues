...
fn update_title_only_show_reports_new_title() {
    ...
    let out = run_iss(&repo, &["update", &id, "--title", "after"]);
    assert!(
        out.status.success(),
        "iss update --title failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
...
fn update_three_fields_lands_one_commit_with_three_trailers() {
    ...
    assert!(
        out.status.success(),
        "iss update three-fields failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn update_assignee_set_then_unset_round_trip() {
    ...
    // Set.
    let out = run_iss(&repo, &["update", &id, "--assignee", "alice"]);
    assert!(
        out.status.success(),
        "iss update --assignee failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let out = run_iss(&repo, &["show", &id]);
    ...
    // Unset.
    let out = run_iss(&repo, &["update", &id, "--unset-assignee"]);
    assert!(
        out.status.success(),
        "iss update --unset-assignee failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let out = run_iss(&repo, &["show", &id]);
    ...
    assert!(
        ...
        "show after unset should omit the assignee line, got: {stdout}"
    );
...
fn update_json_envelope_shape() {
    ...
    assert!(
        out.status.success(),
        "iss update --json failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn update_no_field_flags_exits_two_with_hint() {
    ...
    let out = run_iss(&repo, &["update", &id]);
...
fn update_nonexistent_id_exits_one() {
    ...
    let out = run_iss(&repo, &["update", nonexistent, "--title", "x"]);
    assert!(
        !out.status.success(),
...
fn update_bad_id_exits_two() {
    ...
    for bad in ["short", "GGGGGGG", "12345", "not-a-real-bug-id"] {
        let out = run_iss(&repo, &["update", bad, "--title", "x"]);
...
fn update_in_non_jj_directory_exits_two() {
    ...
    let out = run_iss(&dir, &["update", "abcdef0", "--title", "x"]);
...
fn update_in_jj_repo_without_bugs_bookmark_exits_two_with_init_hint() {
    let repo = make_git_repo("update_no_bookmark");
    let out = run_iss(&repo, &["update", "abcdef0", "--title", "x"]);
    assert!(!out.status.success());
    assert_eq!(
        out.status.code(),
        Some(2),
