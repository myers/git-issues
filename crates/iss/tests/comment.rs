...
mod common;
use common::{run_iss, run_iss_with_stdin, scratch, scratch_non_git, ISS_BIN};
...
fn comment_json_envelope_shape() {
    ...
    assert!(
        out.status.success(),
        "iss comment --json failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn comment_author_override_appears_in_show() {
    ...
    assert!(
        out.status.success(),
        "iss comment with --author failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
...
fn comment_two_in_a_row_chronological_order() {
    ...
    let out =
        run_iss_with_stdin(&repo, &["comment", &id, "-F", "-"], b"FIRST in time");
    assert!(
        out.status.success(),
    ...
    let out =
        run_iss_with_stdin(&repo, &["comment", &id, "-F", "-"], b"SECOND in time");
    assert!(
        out.status.success(),
...
fn comment_empty_body_exits_two() {
    ...
    let out = run_iss_with_stdin(&repo, &["comment", &id, "-F", "-"], b"");
    assert!(!out.status.success(), "empty-body comment must fail");
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn comment_in_non_jj_directory_exits_two() {
    ...
    let out =
        run_iss_with_stdin(&dir, &["comment", "abcdef0", "-F", "-"], b"some body");
...
fn comment_in_jj_repo_without_bugs_bookmark_exits_two_with_init_hint() {
    let repo = make_git_repo("comment_no_bookmark");
    let out = run_iss_with_stdin(
        &repo,
        &["comment", "abcdef0", "-F", "-"],
        b"some body",
