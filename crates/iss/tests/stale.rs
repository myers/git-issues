...
fn run_iss_with_env(cwd: &Path, args: &[&str], clock_secs: u64) -> Output {
    Command::new(ISS_BIN)
        .args(args)
...
fn run_iss_with_stdin_and_env(
    ...
    child
        ...
        .expect("write stdin");
    child.wait_with_output().expect("wait for iss")
}
...
fn create_issue_at(
    ...
    let out = run_iss_with_stdin_and_env(repo, &args, body, clock_secs);
    assert!(
        out.status.success(),
...
fn stale_default_days_14_returns_old_issues() {
    ...
    let out = run_iss_with_env(&repo, &["stale"], NOW);
    assert!(
        out.status.success(),
...
fn stale_days_flag_widens_window() {
    ...
    // --days 3: both.
    let out = run_iss_with_env(&repo, &["stale", "--days", "3"], NOW);
    let rows = parse_stale_rows(&String::from_utf8_lossy(&out.stdout));
    assert_eq!(rows.len(), 2);

    // --days 999: neither.
    let out = run_iss_with_env(&repo, &["stale", "--days", "999"], NOW);
    let rows = parse_stale_rows(&String::from_utf8_lossy(&out.stdout));
...
fn stale_json_shape_is_bare_array() {
    ...
    let out = run_iss_with_env(&repo, &["--json", "stale"], NOW);
...
fn stale_default_status_open_excludes_closed() {
    ...
    let out = run_iss_with_env(&repo, &["close", &id], NOW - 30 * DAY);
    assert!(out.status.success());
    ...
    let out = run_iss_with_env(&repo, &["stale", "--status", "closed"], NOW);
    let rows = parse_stale_rows(&String::from_utf8_lossy(&out.stdout));
    assert_eq!(rows.len(), 1);
    ...
    let out = run_iss_with_env(&repo, &["stale", "--status", "all"], NOW);
    let rows = parse_stale_rows(&String::from_utf8_lossy(&out.stdout));
    assert_eq!(rows.len(), 2);
}
...
fn stale_limit_truncates() {
    ...
    let out = run_iss_with_env(&repo, &["stale", "--limit", "2"], NOW);
    let rows = parse_stale_rows(&String::from_utf8_lossy(&out.stdout));
    assert_eq!(rows.len(), 2);
    ...
    let out = run_iss_with_env(&repo, &["stale", "--limit", "0"], NOW);
    let rows = parse_stale_rows(&String::from_utf8_lossy(&out.stdout));
...
fn stale_age_render_days_form() {
    ...
    let out = run_iss_with_env(&repo, &["stale", "--days", "7"], NOW);
    let rows = parse_stale_rows(&String::from_utf8_lossy(&out.stdout));
    assert_eq!(rows.len(), 1);
...
fn stale_filters_by_meta() {
    ...
    let out = run_iss_with_env(&repo, &["stale", "--days", "14", "--meta", "team=infra"], NOW);
    assert!(
        out.status.success(),
...
fn stale_type_filter_composes() {
    ...
    let out = run_iss_with_env(&repo, &["stale", "--type", "bug"], NOW);
    let rows = parse_stale_rows(&String::from_utf8_lossy(&out.stdout));
    assert_eq!(rows.len(), 1);
