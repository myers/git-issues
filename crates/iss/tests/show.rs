...
fn show_json_error_envelope_on_nonexistent_id() {
    ...
    let out = run_iss(&repo, &["--json", "show", nonexistent]);
...
fn show_with_nonexistent_id_exits_one() {
    ...
    let out = run_iss(&repo, &["show", nonexistent]);
...
fn show_in_non_jj_directory_exits_two() {
    ...
    let out = run_iss(&dir, &["show", "abcdef0"]);
...
fn show_in_jj_repo_without_bugs_bookmark_exits_two_with_init_hint() {
    ...
    // should fire with the typed `run iss init first` message rather
    // than the raw jj stderr from a downstream read attempt.
    let repo = make_git_repo("show_no_bookmark");

    let out = run_iss(&repo, &["show", "abcdef0"]);
    assert!(!out.status.success());
    assert_eq!(
        out.status.code(),
        Some(2),
...
fn show_plain_renders_metadata_block() {
    ...
    let show = run_iss(&repo, &["show", id_str]);
    assert!(
        show.status.success(),
        "iss show failed: {}",
