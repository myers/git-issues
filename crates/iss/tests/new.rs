...
#[test]
fn new_without_iss_init_first_exits_two_with_run_iss_init_first_message() {
    // Fresh git repo, but no `issues` bookmark — we expect a typed
    // preflight error, NOT the raw git stderr we'd get from trying to
    // write against an empty `bookmarks(issues)` revset.
    let repo = make_git_repo("new_no_bookmark");
...
fn new_in_non_jj_directory_exits_two_with_not_a_jj_repo_message() {
    ...
    let out = run_iss_with_stdin(&dir, &["new", "-t", "x", "-F", "-"], b"");
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
fn new_full_field_round_trip() {
    ...
    assert!(
        ...
        "second iss new failed: code={:?} stderr={}",
        second.status.code(),
...
fn new_with_parent_and_dep_compose() {
    ...
    let epic = run_iss_with_stdin(&repo, &["new", "-t", "Epic", "--type", "epic", "-F", "-"], b"");
    let epic_id = parse_id_from_plain_stdout(&epic.stdout);
    let blocker = run_iss_with_stdin(&repo, &["new", "-t", "blocker", "-F", "-"], b"");
...
fn new_with_no_file_flag_creates_bug_with_empty_body() {
    ...
    let out = run_iss(&repo, &["new", "-t", "empty body bug"]);
    assert!(
        out.status.success(),
        "iss new with no -F failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr),
    );
    let id = parse_id_from_plain_stdout(&out.stdout);
...
fn new_meta_seeds_metadata_atomically() {
    ...
    let show = run_iss(&repo, &["show", id_str, "--json"]);
    assert!(
        show.status.success(),
        "iss show --json failed: {}",
