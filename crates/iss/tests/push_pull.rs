...
//! drive the compiled binary against per-test scratch git-clones-of-a-
...
//! as the "remote" and one or two git clones; the `iss` binary lives
...
mod common;
use common::{run_iss, scratch_non_git, ISS_BIN};
...
fn push_pull_single_clone_round_trip() {
    ...
    must_succeed(&run_iss(&alice, &["init"]), "iss init (alice)");
    let new_out = run_iss(&alice, &["new", "-t", "shared title"]);
    must_succeed(&new_out, "iss new (alice)");
    let id = String::from_utf8_lossy(&new_out.stdout).trim().to_owned();
    assert_eq!(id.len(), 7, "id must be 7 hex chars: {id:?}");
    let push = run_iss(&alice, &["push", "origin"]);
    must_succeed(&push, "iss push origin (alice)");
    ...
    let pull = run_iss(&bob, &["pull", "origin"]);
    must_succeed(&pull, "iss pull origin (bob)");
    ...
    let show = run_iss(&bob, &["show", &id]);
    must_succeed(&show, "iss show (bob)");
...
fn push_json_envelope_shape() {
    ...
    let alice = root.join("alice");
    must_succeed(&run_iss(&alice, &["init"]), "init");
    must_succeed(&run_iss(&alice, &["new", "-t", "x"]), "new");
    let out = run_iss(&alice, &["--json", "push", "origin"]);
...
fn pull_json_envelope_with_no_remote_data_yet() {
    ...
    let alice = root.join("alice");
    let out = run_iss(&alice, &["--json", "pull", "origin"]);
    ...
    // Plain-text variant.
    let out = run_iss(&alice, &["pull", "origin"]);
...
fn pull_two_clones_same_field_lww_converges() {
    ...
    must_succeed(&run_iss(&alice, &["init"]), "init (alice)");
    let new_out = run_iss(&alice, &["new", "-t", "shared"]);
    must_succeed(&new_out, "new (alice)");
    let id = String::from_utf8_lossy(&new_out.stdout).trim().to_owned();
    must_succeed(&run_iss(&alice, &["push", "origin"]), "push (alice)");
    must_succeed(&run_iss(&bob, &["pull", "origin"]), "first pull (bob)");

    // Concurrent edits: both retitle.
    must_succeed(
        &run_iss(&alice, &["update", &id, "--title", "alice title"]),
        "alice update",
    );
    must_succeed(&run_iss(&alice, &["push", "origin"]), "alice push");
    must_succeed(
        &run_iss(&bob, &["update", &id, "--title", "bob title"]),
        "bob update",
    );
...
fn pull_two_clones_different_fields_both_survive() {
    ...
    let id = String::from_utf8_lossy(&new_out.stdout).trim().to_owned();
    must_succeed(&run_iss(&alice, &["push", "origin"]), "alice push");
    must_succeed(&run_iss(&bob, &["pull", "origin"]), "bob first pull");

    must_succeed(
        &run_iss(&alice, &["label", "add", &id, "alice-label"]),
        "alice add label",
    );
    must_succeed(&run_iss(&alice, &["push", "origin"]), "alice push 2");
    must_succeed(
        &run_iss(&bob, &["label", "add", &id, "bob-label"]),
        "bob add label",
    );
    must_succeed(&run_iss(&bob, &["pull", "origin"]), "bob pull");
...
fn pull_two_clones_different_issues_no_merge() {
    ...
    let show_a = run_iss(&bob, &["show", &a_id]);
    must_succeed(&show_a, "bob show alice issue");
    let show_b = run_iss(&bob, &["show", &b_id]);
...
fn push_unknown_remote_exits_two_with_remote_not_found_kind() {
    ...
    must_succeed(&run_iss(&alice, &["init"]), "init");
    must_succeed(&run_iss(&alice, &["new", "-t", "x"]), "new");
    let out = run_iss(&alice, &["--json", "push", "nope"]);
...
fn pull_unknown_remote_exits_two() {
    ...
    let alice = root.join("alice");
    let out = run_iss(&alice, &["pull", "nope"]);
...
fn push_outside_jj_repo_exits_two_not_a_jj_repo() {
    ...
    let out = run_iss(&dir, &["push", "origin"]);
...
fn pull_outside_jj_repo_exits_two_not_a_jj_repo() {
    ...
    let out = run_iss(&dir, &["pull", "origin"]);
    assert!(!out.status.success());
    assert_eq!(out.status.code(), Some(2));
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
fn push_without_init_exits_two_missing_marker() {
    ...
    let alice = root.join("alice");
    let out = run_iss(&alice, &["push", "origin"]);
...
fn pull_first_time_bootstraps_v3_sentinel() {
    ...
    let bob = root.join("bob");
    must_succeed(&run_iss(&alice, &["init"]), "init (alice)");
    must_succeed(&run_iss(&alice, &["new", "-t", "from alice"]), "new alice");
    must_succeed(&run_iss(&alice, &["push", "origin"]), "push alice");
...
fn pull_clean_round_trip_is_fast_forwards_only() {
    ...
    must_succeed(&run_iss(&alice, &["init"]), "init (alice)");
    let id_out = run_iss(&alice, &["new", "-t", "thing"]);
    must_succeed(&id_out, "new (alice)");
    let id = String::from_utf8_lossy(&id_out.stdout).trim().to_owned();
    must_succeed(&run_iss(&alice, &["push", "origin"]), "push 1");
    must_succeed(&run_iss(&bob, &["pull", "origin"]), "first pull (bob)");

    // Alice mutates; nobody else does.
    must_succeed(
        &run_iss(&alice, &["update", &id, "--title", "renamed"]),
        "alice rename",
    );
    must_succeed(&run_iss(&alice, &["push", "origin"]), "push 2");
...
fn pull_meta_sentinel_divergence_is_noop_not_error() {
    ...
    must_succeed(&run_iss(&alice, &["init"]), "init (alice)");
    let new_out = run_iss(&alice, &["new", "-t", "alice's issue"]);
    must_succeed(&new_out, "new (alice)");
    let id = String::from_utf8_lossy(&new_out.stdout).trim().to_owned();
    must_succeed(&run_iss(&alice, &["push", "origin"]), "alice push");
    ...
    must_succeed(&run_iss(&bob, &["init"]), "init (bob)");
...
fn push_meta_sentinel_non_fast_forward_succeeds_with_data() {
    ...
    let push2 = run_iss(&bob, &["push", "origin"]);
...
fn init_writes_jjf_fetch_refspec_for_existing_remotes() {
    ...
    must_succeed(&run_iss(&alice, &["init"]), "init (alice)");
    let new_out = run_iss(&alice, &["new", "-t", "from alice"]);
    must_succeed(&new_out, "new (alice)");
    let id = String::from_utf8_lossy(&new_out.stdout).trim().to_owned();
    must_succeed(&run_iss(&alice, &["push", "origin"]), "alice push");

    // Sanity: bob's fresh clone has the standard heads-only refspec
    // and no refs/jjf/* refspec.
    ...
    assert!(
        ...
        "precondition: bob's clone should not have a refs/jjf/* fetch refspec yet; got:\n{pre_text}"
    );
    ...
    assert!(
        ...
        "init should have planted the refs/jjf/* fetch refspec; got:\n{post_text}"
    );

    // Re-running `iss init` is idempotent: the refspec is not
    // duplicated.
    must_succeed(&run_iss(&bob, &["init"]), "init (bob) #2");
...
/// `crates/iss-storage/tests/integration.rs` pins the write-path
...
fn metadata_lww_under_divergent_remote_tips() {
    ...
    must_succeed(
        &run_iss(&alice, &["metadata", "set", &id, "gc.kind", "alice-value"]),
        "alice set metadata",
    );
    must_succeed(&run_iss(&alice, &["push", "origin"]), "alice push 2");
    ...
    must_succeed(
        &run_iss(&bob, &["metadata", "set", &id, "gc.kind", "bob-value"]),
        "bob set metadata",
    );
    ...
    let show = run_iss(&bob, &["show", "--json", &id]);
    must_succeed(&show, "bob show --json");
    let v: serde_json::Value =
        serde_json::from_slice(&show.stdout).expect("iss show --json must emit valid JSON");
