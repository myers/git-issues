...
fn add_comment(repo: &Path, id: &str, body: &[u8]) {
    ...
    assert!(
        out.status.success(),
        "iss comment failed: code={:?} stderr={}",
        out.status.code(),
        String::from_utf8_lossy(&out.stderr)
    );
}
...
fn search_title_hit_emits_one_row() {
    ...
    let out = run_iss(&repo, &["search", "segfault"]);
    assert!(
        out.status.success(),
...
fn search_json_envelope_shape() {
    ...
    let out = run_iss(&repo, &["search", "widget", "--json"]);
    assert!(
        out.status.success(),
...
fn search_empty_query_plain_is_silent_json_is_empty_envelope() {
    ...
    let out = run_iss(&repo, &["search", "", "--json"]);
...
fn search_label_and_status_filters_compose_with_query() {
    ...
    let _ = run_iss(&repo, &["close", &closed]);

    // No filter: all three hit.
    let out = run_iss(&repo, &["search", "widget"]);
    assert!(out.status.success());
    ...
    let out = run_iss(&repo, &["search", "widget", "--status", "open"]);
    assert!(out.status.success());
    let rows = parse_search_rows(&String::from_utf8_lossy(&out.stdout));
...
fn search_include_comments_widens_result_set() {
    ...
    let out = run_iss(&repo, &["search", "needle"]);
    assert!(out.status.success());
    let rows = parse_search_rows(&String::from_utf8_lossy(&out.stdout));
    ...
    let out = run_iss(&repo, &["search", "needle", "--include-comments"]);
    assert!(out.status.success());
    let rows = parse_search_rows(&String::from_utf8_lossy(&out.stdout));
...
fn search_limit_truncates_after_sort() {
    ...
    let out = run_iss(&repo, &["search", "widget", "--limit", "2"]);
    assert!(out.status.success());
    let rows = parse_search_rows(&String::from_utf8_lossy(&out.stdout));
...
fn search_outside_jj_repo_is_preflight_failure() {
    ...
    let out = run_iss(&dir, &["search", "anything"]);
...
fn search_excludes_metadata_by_default() {
    ...
    let out = run_iss(&repo, &["search", "unique-needle-xyz", "--json"]);
    assert!(out.status.success());
    let stdout = String::from_utf8_lossy(&out.stdout);
...
fn search_parent_flag_intersects_with_query() {
    ...
    let epic_id = parse_id_from_stdout(
        &run_iss(&repo, &["new", "--json", "-t", "Epic", "--type", "epic", "--slug", "epic"]).stdout,
    );
    // Two children, both with "needle" in the title — only one is parented.
    let parented_id = parse_id_from_stdout(
        &run_iss(&repo, &["new", "--json", "-t", "needle child", "--parent", epic_id.as_str()]).stdout,
    );
    let _orphan = run_iss(&repo, &["new", "--json", "-t", "needle orphan"]);

    let out = run_iss(&repo, &["search", "--json", "--parent", "epic", "needle"]);
...
fn search_parent_bad_hex_exits_one_issue_not_found() {
    ...
    let out = run_iss(&repo, &["--json", "search", "--parent", "deadbee", "anything"]);
    assert!(
        !out.status.success(),
